...
to one built on top of NDB Cluster's Read-Committed isolation level and TiDB's Snapshot Isolation.
...
serializability implementation's issues, including a starvation problem, are explained in Appendix D. The 2nd-tier serializability implementation
on top of TiDB's Snapshot Isolation is also explained in Appendix D.

NDB Cluster can scale up to 144 data nodes now and it is an in-memory database, this makes the status of the last two requirements 
look pretty good. The 2nd-tier serializability implementation I've developed for NDB Cluster will not only provide consistency, but also 
...
demonstrate us that it satisfies all three requirements in the peer-review process I'll host here. If it would, the scalability of NDB Cluster
also implies it could cover the low-end 80% of applications too and the conflict serializability problem could have been solved; If 
...
system which I'll develop in subsequent work). Also, this framework can explain all six serializability implementations make it a candidate 
to replace the relevant part of the current SQL standard, which only allows 2PL.

The 2nd-tier serializability implementation on top of TiDB's Snapshot Isolation is a second attempt. It is based on the generalization of
Serializable Snapshot Isolation to a distributed database system I've developed in Section 5.

The content in the ConflictSerializability.html file represents the current status of the project.
...
NDB Cluster and TiDB  and give me some feedback.
